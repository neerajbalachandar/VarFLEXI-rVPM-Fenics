...
print(
    f"Nonlinear solid v17: span={span} m, c_root={root_chord} m, c_tip={tip_chord} m, "
...
def internal_virtual_work(r, u_test):
    return inner(first_pk_stress(r), grad(u_test)) * dx
...
def k(u_trial, u_test):
    base = inner(sigma_linear(u_trial), sym(grad(u_test))) * dx
...
out_dir = os.path.join(repo_root, "results", "solid", "v17")
...
for i_step in range(Nsteps):
    ...
    try:
        ...
        raise RuntimeError(f"Nonlinear solid solve failed at step {i_step + 1}/{Nsteps}: {err}")
    if i_step == 0 or (i_step + 1) % 20 == 0:
        print(
            f"Solid step {i_step + 1}/{Nsteps}: Newton converged in {n_it} iterations "
...
diag_csv = os.path.join(out_dir, "solid_v17_diagnostics.csv")
