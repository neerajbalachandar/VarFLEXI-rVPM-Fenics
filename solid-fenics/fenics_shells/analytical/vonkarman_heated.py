...
def analytical_solution(Ai, Di, a_rad, b_rad):
    ...
    # since kTx = kTy = h, hQ as in the reference paper
    hQ = np.sqrt(2*eta)*(1.0 + 2.0*eta + mu)/(1.0 + mu)
    # since kTx = kTy = h, hP+ as in the reference paper
    hP_pos = 2*np.sqrt(1.0 - mu)/(1.0 + mu)
    h_cr = np.minimum(hQ, hP_pos)

    # - characteristic radius for lenticular cross-section
    ratio = b_rad/a_rad
    psiq = (1.0/(48.0*np.pi**2))*(1.0 - (nu**2/beta))*ratio**2 / \
        (5.0 + 2.0*((1.0/rho) - (nu/beta))*ratio**2 + (5.0/beta)*ratio**4)
    ...
    ls_f = [(1.0 + mu)*(np.sqrt(3.0)*np.sqrt(27*j**2 + 4.0*(1.0 + mu)) + 9.0*j)
            for j in h_before]
    ls_Kbefore = [(2.0**(1.0/3.0)*i**(2.0/3.0) - 2.0*3.0**(1.0/3.0)
                   * (1.0 + mu))/(6.0**(2.0/3.0)*i**(1.0/3.0)) for i in ls_f]
    ls_K1after = [j/2.0*(1.0 + mu) + np.sqrt(j*j/4.0 *
                                             (1.0 + mu)*(1.0 + mu) - (1.0 - mu)) for j in h_after]
    ls_K2after = [j/2.0*(1.0 + mu) - np.sqrt(j*j/4.0 *
                                             (1.0 + mu)*(1.0 + mu) - (1.0 - mu)) for j in h_after]
