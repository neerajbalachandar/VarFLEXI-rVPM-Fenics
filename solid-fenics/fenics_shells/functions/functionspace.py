...
class ProjectedFunctionSpace(dolfin.FunctionSpace):
    def __init__(self, *args, **kwargs):
        ...
        self._projected_element = ufl.MixedElement(self._full_element.sub_elements()[
                                                   0:-self._num_projected_subspaces])

        # Construct projected space (self)
        dolfin.FunctionSpace.__init__(
            self, args[0], self._projected_element, **kwargs)
        # Construct projected space
        self._full_function_space = FullFunctionSpace(
            self.mesh(), self._full_element, **kwargs)
